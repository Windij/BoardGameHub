...
from wtforms import SelectField, StringField, IntegerField, SubmitField, DateField
...
class GameSessionForm(FlaskForm):
    ...
    data = DateField("Время встречи", validators=[DataRequired()])
    location = StringField("Место проведения", validators=[DataRequired()])
    max_participants = IntegerField("Максимальное количество участников", validators=[DataRequired()])
